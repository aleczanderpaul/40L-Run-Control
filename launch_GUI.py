...
pressure_log_filepath = '40L_run_control/pressure_log_copy.csv'
create_pressure_log_csv(pressure_log_filepath)
plotter.add_plot(title='Vessel Pressure', x_axis=('Time', 's'), y_axis=('Pressure', 'Pa'), buffer_size=100, csv_filepath=pressure_log_filepath, datatype='pressure')
plotter.start_timer(title='Vessel Pressure', interval_ms=1000)

pressure_log_2_filepath = '40L_run_control/pressure_log_copy_2.csv'
create_pressure_log_csv(pressure_log_2_filepath)
plotter.add_plot(title='Vessel Pressure 2', x_axis=('Time', 's'), y_axis=('Pressure', 'Pa'), buffer_size=100, csv_filepath=pressure_log_2_filepath, datatype='pressure')
plotter.start_timer(title='Vessel Pressure 2', interval_ms=1000)

plotter.add_command_button(title='Log Pressure', command=f'.venv\Scripts\python.exe 40L_run_control/log_pressure.py {pressure_log_filepath} COM4 2')
